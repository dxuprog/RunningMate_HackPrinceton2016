...
def sendToPi(p):
    global _bRight
    print("we have pose: ", p)
    if (int(p) == 4):
         _bRight = not(_bRight)

def blinkRight():
    while (True):
        time.sleep(_blinking_speed)
        while (_bRight):
            GPIO.output(_bRight_pin, True)
            time.sleep(_blinking_speed)
            GPIO.output(_bRight_pin, False)
            time.sleep(_blinking_speed)

if __name__ == '__main__':
    import subprocess
    try:
        GPIO.setup(_bRight_pin, GPIO.OUT)
        m = Myo(NNClassifier(), sys.argv[1] if len(sys.argv) >= 2 else None)
        m.add_raw_pose_handler(sendToPi)
        blinkRight = thread.start_new_thread(blinkRight, ())
        m.connect()
